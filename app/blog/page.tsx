...
export default function BlogPage() {
  return <main>
    ...
    <section className="section"><div className="wrap blog-grid">{posts.map((post) => <article className="blog-card" key={post.slug}>
      ...
      <div className="blog-card-copy"><div className="kicker">{post.category} · {post.readTime} · {post.date}</div><h2><Link href={`/blog/${post.slug}`}>{post.title}</Link></h2><p>{post.description}</p><Link href={`/blog/${post.slug}`} className="text-link">Read article →</Link></div>
    </article>)}</div></section>
  </main>;
}
